...
PORTA = 5578
PASTA_BASE = r'C:\CIGS'
PASTA_DOWNLOAD = r"C:\CIGS\Downloads"
ARQUIVO_LOG_DEBUG = r"C:\CIGS\CIGS_debug.log"
UNRAR_PATH = r"C:\CIGS\UnRAR.exe"
...
def ajustar_permissoes():
    ...
    try:
        # Dá controle total para Todos na pasta CIGS (Ambiente controlado/Intranet)
...
def agendar_tarefa_universal(url, nome_arquivo, data_hora, usuario, senha, start_in, sistema, modo="COMPLETO"):
    ...
    pasta_destino = start_in or MAPA_SISTEMAS.get(sistema.upper(), r"C:\CIGS")
    ...
    log_bat = r"%TEMP%\cigs_launcher.log" # Log no TEMP do usuário
...
"""
    ...
    try:
        d, h = data_hora.split(" ")
        task = f"CIGS_{sistema}_{datetime.now().strftime('%d%H%M')}"
...
def executar():
    ...
    path_calc = os.path.join(MAPA_SISTEMAS.get(sist, r"C:\CIGS"), "Atualizadores", sist)
...
def abortar():
    try: subprocess.run('schtasks /delete /tn "CIGS*" /f', shell=True)
    except: pass
